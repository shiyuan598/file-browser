<template>
    <div class="shell" :class="{ ['shell--hidden']: !showShell }" :style="{ height: `${this.shellHeight}em` }">
        <div
            @pointerdown="startDrag()"
            @pointerup="stopDrag()"
            class="shell__divider"
            :style="this.shellDrag ? { background: `${checkTheme()}` } : ''"></div>
        ...
        <div @pointerup="stopDrag()" class="shell__overlay" v-show="this.shellDrag"></div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        handleDrag: throttle(function (event) {
            ...
            const bottom = 2.25 + document.querySelector(".shell__divider").offsetHeight / this.fontsize;

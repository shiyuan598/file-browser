...
export async function upload(filePath, content = "", overwrite = false, onupload) {
    if (!tusSettings) {
...
async function createUpload(resourcePath) {
    ...
    if (headResp.status !== 201) {
        throw new Error(`Failed to create an upload: ${headResp.status} ${headResp.statusText}`);
    }
}
...
function computeETA(state) {
    ...
    const uploadedSize = state.progress.reduce((acc, progress) => acc + progress, 0);
...
function calcProgress(filePath) {
    ...
    let bytesSinceLastUpdate = fileData.currentBytesUploaded - fileData.initialBytesUploaded;
    ...
    let avgRecentSpeed = fileData.sumOfRecentSpeeds / fileData.recentSpeeds.length;
    fileData.currentAverageSpeed = ALPHA * avgRecentSpeed + ONE_MINUS_ALPHA * fileData.currentAverageSpeed;
